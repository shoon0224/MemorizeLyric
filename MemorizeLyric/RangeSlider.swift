...
class RangeSliderTrackLayer: CALayer {
    ...
    override func draw(in ctx: CGContext) {
        ...
        let rect = CGRect(x: lowerValuePosition, y: 0.0, width: upperValuePosition - lowerValuePosition, height: bounds.height)// 슬라이더 사이에 색상을 y축으로 얼마나 칠할 것인지
        ctx.fill(rect)
    }
}
...
class RangeSliderThumbLayer: CALayer {
    ...
    var strokeColor: UIColor = UIColor.gray { // 슬라이더 동그라미 태두리 색상
        didSet {
            setNeedsDisplay()
        }
    }
    var lineWidth: CGFloat = 0.1 { // 슬라이더 동그라미 태두리 두께
        didSet {
            setNeedsDisplay()
        }
    }
    ...
    override func draw(in ctx: CGContext) {
        ...
        if highlighted {
            ctx.setFillColor(UIColor(white: 0.0, alpha: 0.1).cgColor) //슬라이더 원을 잡고 드레그 할때 색깔 진하기
            ctx.addPath(thumbPath.cgPath)
            ctx.fillPath()
        }
    }
...
public class RangeSlider: UIControl {
    ...
    @IBInspectable public var minimumValue: Double = 0  {// 슬라이더의 최소값
        willSet(newValue) {
    ...
    @IBInspectable public var lowerValue: Double = 0 { //초기값
        didSet {
